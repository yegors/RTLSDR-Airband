...
bool srt_stream_init(srt_stream_data* sdata, mix_modes mode, size_t len) {
    ...
    sdata->ep_id = srt_epoll_create();
    if (sdata->ep_id < 0) {
        log(LOG_ERR, "srt_stream: epoll_create failed: %s\n", srt_getlasterror_str());
    ...
    if (srt_epoll_add_usock(sdata->ep_id, sdata->listen_socket, &events) == SRT_ERROR) {
        log(LOG_ERR, "srt_stream: epoll_add_usock failed: %s\n", srt_getlasterror_str());
        srt_close(sdata->listen_socket);
        srt_epoll_release(sdata->ep_id);
        sdata->listen_socket = SRT_INVALID_SOCK;
        sdata->ep_id = -1;
        return false;
    }
...
static void srt_stream_accept(srt_stream_data* sdata) {
    ...
    SRT_EPOLL_EVENT ev;
    int ret = srt_epoll_uwait(sdata->ep_id, &ev, 1, 0);
    if (ret <= 0)
        return;
    if (ev.fd != sdata->listen_socket || !(ev.events & SRT_EPOLL_IN))
        return;
